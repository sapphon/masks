...
public class MSACCMobileInputs : MonoBehaviour
{
    [Header("Controller")]
    [Tooltip(
        "Here you must associate a camera controller of type 'MSCameraController' that will receive the inputs of this object.")]
    ...
    [Header("Settings")]
    [Tooltip(
        "Here it is possible to define the speed at which the variable that controls the scroll will be incremented or decremented by the buttons.")]
    [Range(0.1f, 1.0f)]
    ...
    private void Awake()
    {
        ...
        if (cameraController)
        {
            ...
            if (!joystick)
            {
                Debug.LogWarning("The prefab " + transform.root.name +
                                 " had its structure modified and this interferes in the correct functioning of the code. The controller will be disabled to avoid problems.");
                error = true;
        ...
        {
            Debug.LogWarning("No 'camera controller' was associated to object " + transform.root.name +
                             ", so it was disabled from the scene.");
            error = true;
            transform.root.gameObject.SetActive(false);
        }
    }
    ...
    private void Update()
    {
        if (!error)
        {
            ...
            if (invertJoystickX) joystickInput = new Vector2(-joystickInput.x, joystickInput.y);
            if (invertJoystickY) joystickInput = new Vector2(joystickInput.x, -joystickInput.y);
