...
public class MSACC_CameraSetting
{
    ...
    [Header("Update mode")]
    [Tooltip(
        "Here it is possible to decide whether the motion of the cameras will be processed in the void Update, FixedUpdate or LateUpdate. The mode that best suits most situations is the 'LateUpdate'.")]
    ...
    [Header("Configure Inputs")] [Tooltip("The input that will define the horizontal movement of the cameras.")]
...
public class MSACC_SettingsCameraFirstPerson
{
    ...
    [Space(7)] [Tooltip("If this variable is true, the X-axis input will be inverted.")]
    ...
    [Tooltip(
        "If this variable is true, the camera will only rotate when the key selected in the 'KeyToRotate' variable is pressed. If this variable is false, the camera can rotate freely, even without pressing any key.")]
    public bool rotateWhenClick;

    [Header("Sensibility")] [Range(1, 20)] [Tooltip("Horizontal camera rotation sensitivity.")]
...
{
    [Header("Rotation")] [Tooltip("If this variable is true, the code makes a lookAt using quaternions.")]
    public bool customLookAt;

    [Header("Movement")] [Range(1, 20)] [Tooltip("The speed at which the camera can follow the player.")]
    ...
    [Header("Use Scrool")]
    [Tooltip(
        "If this variable is true, the 'FollowPlayer' camera style will have the player's distance affected by the mouse scrool. This will allow the player to zoom in or out of the camera.")]
...
public class MSACC_SettingsCameraOrbital
{
    ...
    [Range(0.01f, 2.0f)]
    [Tooltip(
        "In this variable you can configure the sensitivity with which the script will perceive the movement of the X and Y inputs. ")]
...
public class MSACC_SettingsCameraOrbitalThatFollows
{
    ...
    [Header("Settings(Follow)")] [Range(1, 20)] [Tooltip("The speed at which the camera can follow the player.")]
    ...
    [Tooltip(
        "In this variable it is possible to define how the control will be redefined for the camera that follows the player, through input or through a time.")]
    public ResetTimeType ResetControlType = ResetTimeType.Time;

    [Tooltip(
        "If 'ResetControlType' is set to 'Input_OnlyWindows', the key that must be pressed to reset the control will be set by this variable.")]
    ...
    [Range(1.0f, 50.0f)]
    [Tooltip(
        "If 'ResetControlType' is set to 'Time', the wait time for the camera to reset the control will be set by this variable.")]
...
public class MSACC_SettingsCameraETS_StyleCamera
{
    ...
    [Header("Settings")] [Range(1, 20)] [Tooltip("Horizontal camera rotation sensitivity.")]
    public float sensibilityX = 10.0f;
...
public class MSACC_SettingsFlyCamera
{
    ...
    [Tooltip(
        "Here you can configure the keys that must be pressed to accelerate the movement of the camera 'CameraFly'.")]
...
{
    [HideInInspector] public bool _enableMobileInputs;


    //global inputs
    [HideInInspector] public float _horizontalInputMSACC;

    [HideInInspector] public int _mobileInputsIndex; // 0 = off,   1 = all,   2 = scroll buttons only

    [HideInInspector] public float _scrollInputMSACC;

    [HideInInspector] public float _verticalInputMSACC;
    ...
    [Tooltip(
        "Here you must associate the object that the cameras will follow. If you leave this variable empty, the cameras will follow the object in which this script was placed.")]
    ...
    private void Awake()
    {
        ...
        var temp = new GameObject("PlayerCams");
        ...
        for (var x = 0; x < cameras.Length; x++)
            ...
            {
                if (cameras[x].volume == 0) cameras[x].volume = 1;
                ...
                if (cameras[x].rotationType == MSACC_CameraType.TipoRotac.FollowPlayer)
                {
                    ...
                    if (CameraSettings.ajustTheLayers)
                    {
                        ...
                        foreach (var trans in targetTransform.gameObject.GetComponentsInChildren<Transform>(true))
                            trans.gameObject.layer = 2;
                    }

                    camFollowPlayerDistance[x] =
                        Vector3.Distance(cameras[x]._camera.transform.position, targetTransform.position);
                }
                ...
                distanceFromOrbitalCamera[x] =
                    Vector3.Distance(cameras[x]._camera.transform.position, targetTransform.position);
                distanceFromOrbitalCamera[x] = Mathf.Clamp(distanceFromOrbitalCamera[x],
                    CameraSettings.orbital.minDistance, CameraSettings.orbital.maxDistance);
                ...
                var audListner = cameras[x]._camera.GetComponent<AudioListener>();
                if (audListner == null) cameras[x]._camera.transform.gameObject.AddComponent(typeof(AudioListener));
            }
            else
            {
        ...
        if (CameraSettings.followPlayer.minDistance > CameraSettings.followPlayer.maxDistance)
        {
            ...
            Debug.LogWarning(
                "The minimum distance from the 'FollowPlayer' camera is greater than the maximum distance, and this may cause errors.");
        }
    }
            ...
            for (var x = 0; x < cameras.Length; x++)
                if (cameras[x]._camera)
                {
                    if (x == index)
                    {
                        ...
                        if (cameras[x].rotationType == MSACC_CameraType.TipoRotac.OrbitalThatFollows) tempoOrbit = 0.0f;
                        if (cameras[x].rotationType == MSACC_CameraType.TipoRotac.ETS_StyleCamera)
                        {
                            rotacXETS = 0.0f;
                            rotacYETS = 0.0f;
    ...
    private void ManageCameras()
    {
        if (changeCam)
        {
            ...
            for (var x = 0; x < cameras.Length; x++)
            {
                ...
                //
                if (cameras[x].rotationType == MSACC_CameraType.TipoRotac.Orbital ||
                    cameras[x].rotationType == MSACC_CameraType.TipoRotac.OrbitalThatFollows)
                {
                    cameras[x]._camera.transform.LookAt(target);
                    xOrbit[x] = cameras[x]._camera.transform.eulerAngles.y;
                    yOrbit[x] = cameras[x]._camera.transform.eulerAngles.x;
            ...
            if (cameras[index].rotationType == MSACC_CameraType.TipoRotac.Stop ||
                cameras[index].rotationType == MSACC_CameraType.TipoRotac.StraightStop ||
                cameras[index].rotationType == MSACC_CameraType.TipoRotac.LookAtThePlayer ||
                cameras[index].rotationType == MSACC_CameraType.TipoRotac.FlyCamera_OnlyWindows)
                _mobileInputsIndex = 0; // 0 = all mobile inputs off
            if (cameras[index].rotationType == MSACC_CameraType.TipoRotac.FirstPerson ||
                cameras[index].rotationType == MSACC_CameraType.TipoRotac.ETS_StyleCamera ||
                cameras[index].rotationType == MSACC_CameraType.TipoRotac.Orbital || cameras[index].rotationType ==
                MSACC_CameraType.TipoRotac.OrbitalThatFollows)
        ...
        var timeScaleSpeed = Mathf.Clamp(1.0f / Time.timeScale, 0.01f, 1);
        switch (cameras[index].rotationType)
        {
            ...
            case MSACC_CameraType.TipoRotac.StraightStop:
                var linearRotation = Quaternion.LookRotation(
                    objPosicStopCameras[index].transform.position - cameras[index]._camera.transform.position,
                    Vector3.up);
                cameras[index]._camera.transform.rotation = Quaternion.Slerp(cameras[index]._camera.transform.rotation,
                    linearRotation, Time.deltaTime * 15.0f);
                break;
            ...
            case MSACC_CameraType.TipoRotac.FirstPerson:
                //getInputs
                var xInput = _horizontalInputMSACC;
                var yInput = _verticalInputMSACC;
                if (CameraSettings.firstPerson.invertXInput) xInput = -_horizontalInputMSACC;
                if (CameraSettings.firstPerson.invertYInput) yInput = -_verticalInputMSACC;
                ...
                //
                rotacX = ClampAngle(rotacX, -CameraSettings.firstPerson.horizontalAngle,
                    CameraSettings.firstPerson.horizontalAngle);
                rotacY = ClampAngle(rotacY, -CameraSettings.firstPerson.verticalAngle,
                    CameraSettings.firstPerson.verticalAngle);
                var xQuaternion = Quaternion.AngleAxis(rotacX, Vector3.up);
                var yQuaternion = Quaternion.AngleAxis(rotacY, -Vector3.right);
                var _nextRot = originalRotation[index] * xQuaternion * yQuaternion;
                cameras[index]._camera.transform.localRotation = Quaternion.Lerp(
                    cameras[index]._camera.transform.localRotation, _nextRot, Time.deltaTime * 10.0f * timeScaleSpeed);
                //fieldOfView
                cameras[index]._camera.fieldOfView -=
                    _scrollInputMSACC * CameraSettings.firstPerson.speedScroolZoom * 50.0f;
                if (cameras[index]._camera.fieldOfView <
                    initialFieldOfView[index] - CameraSettings.firstPerson.maxScroolZoom)
                    cameras[index]._camera.fieldOfView =
                        initialFieldOfView[index] - CameraSettings.firstPerson.maxScroolZoom;
            ...
            case MSACC_CameraType.TipoRotac.FollowPlayer:
                ...
                {
                    var camLerpSpeed = Time.deltaTime * CameraSettings.followPlayer.displacementSpeed *
                                       (camFollowPlayerDistance[index] * 0.1f);
                    camFollowPlayerDistance[index] = camFollowPlayerDistance[index] -
                                                     _scrollInputMSACC *
                                                     (CameraSettings.followPlayer.scroolSpeed * 50.0f);
                    camFollowPlayerDistance[index] = Mathf.Clamp(camFollowPlayerDistance[index],
                        CameraSettings.followPlayer.minDistance, CameraSettings.followPlayer.maxDistance);
                    var direction = (targetTransform.position - originalPosition[index].transform.position).normalized;
                    var finalPos = targetTransform.position - direction * camFollowPlayerDistance[index];
                    //
                    if (!Physics.Linecast(targetTransform.position, finalPos))
                    {
                        cameras[index]._camera.transform.position =
                            Vector3.Lerp(cameras[index]._camera.transform.position, finalPos, camLerpSpeed);
                    }
                    else if (Physics.Linecast(targetTransform.position, finalPos, out hitCamFollow))
                    {
                        if (CameraSettings.followPlayer.ignoreCollision)
                            ...
                            cameras[index]._camera.transform.position = Vector3.Lerp(
                                cameras[index]._camera.transform.position, hitCamFollow.point, camLerpSpeed);
                    }
                }
                else
                {
                    ...
                    {
                        cameras[index]._camera.transform.position = Vector3.Lerp(
                            cameras[index]._camera.transform.position, originalPosition[index].transform.position,
                            Time.deltaTime * CameraSettings.followPlayer.displacementSpeed);
                    }
                    else if (Physics.Linecast(transform.position, originalPosition[index].transform.position,
                        out hitCamFollow))
                    {
                        if (CameraSettings.followPlayer.ignoreCollision)
                            cameras[index]._camera.transform.position = Vector3.Lerp(
                            ...
                            cameras[index]._camera.transform.position = Vector3.Lerp(
                                cameras[index]._camera.transform.position, hitCamFollow.point,
                                Time.deltaTime * CameraSettings.followPlayer.displacementSpeed);
                    }
                }
                ...
                {
                    var nextRotation =
                        Quaternion.LookRotation(targetTransform.position - cameras[index]._camera.transform.position,
                            Vector3.up);
                    cameras[index]._camera.transform.rotation = Quaternion.Slerp(
                        cameras[index]._camera.transform.rotation, nextRotation,
                        Time.deltaTime * CameraSettings.followPlayer.spinSpeedCustomLookAt);
                }
                else
                {
                    cameras[index]._camera.transform.LookAt(targetTransform.position);
            ...
            case MSACC_CameraType.TipoRotac.Orbital:
                ...
                var minDistance = CameraSettings.orbital.minDistance;
                if (Physics.Linecast(targetTransform.position, cameras[index]._camera.transform.position,
                    out hitCamOrbital))
                    if (!CameraSettings.orbital.ignoreCollision)
                    {
                        distanceFromOrbitalCamera[index] =
                            Vector3.Distance(targetTransform.position, hitCamOrbital.point);
                        minDistance = Mathf.Clamp(distanceFromOrbitalCamera[index], minDistance * 0.5f,
                            CameraSettings.orbital.maxDistance);
                    }

                //getInputs
                var xInputOrb = _horizontalInputMSACC;
                var yInputOrb = _verticalInputMSACC;
                if (CameraSettings.orbital.invertXInput) xInputOrb = -_horizontalInputMSACC;
                if (CameraSettings.orbital.invertYInput) yInputOrb = -_verticalInputMSACC;
                if (CameraSettings.orbital.rotateWhenClick)
                {
                    if (Input.GetKey(CameraSettings.orbital.keyToRotate) || _enableMobileInputs)
                    {
                        xOrbit[index] +=
                            xInputOrb * (CameraSettings.orbital.sensibility * distanceFromOrbitalCamera[index]) /
                            (distanceFromOrbitalCamera[index] * 0.5f);
                        yOrbit[index] -= yInputOrb * CameraSettings.orbital.sensibility *
                                         (CameraSettings.orbital.speedYAxis * 10.0f);
                    }
                }
                else
                {
                ...
                //move - rotation
                yOrbit[index] = ClampAngle(yOrbit[index], CameraSettings.orbital.minAngleY,
                    CameraSettings.orbital.maxAngleY);
                var quatToEuler = Quaternion.Euler(yOrbit[index], xOrbit[index], 0);
                distanceFromOrbitalCamera[index] =
                    Mathf.Clamp(
                        distanceFromOrbitalCamera[index] -
                        _scrollInputMSACC * (CameraSettings.orbital.speedScrool * 50.0f), minDistance,
                        CameraSettings.orbital.maxDistance);
                var zPosition = new Vector3(0.0f, 0.0f, -distanceFromOrbitalCamera[index]);
                var nextPosCam = quatToEuler * zPosition + targetTransform.position;
                var currentPosCam = cameras[index]._camera.transform.position;
                var camRotation = cameras[index]._camera.transform.rotation;
                cameras[index]._camera.transform.rotation =
            ...
            case MSACC_CameraType.TipoRotac.OrbitalThatFollows:
                var movXInput = 0.0f;
                var movYInput = 0.0f;
                var movZInput = _scrollInputMSACC;
                if (CameraSettings.OrbitalThatFollows.rotateWhenClick)
                {
                    if (Input.GetKey(CameraSettings.OrbitalThatFollows.keyToRotate) || _enableMobileInputs)
                    {
                        ...
                        if (CameraSettings.OrbitalThatFollows.invertXInput) movXInput = -_horizontalInputMSACC;
                        if (CameraSettings.OrbitalThatFollows.invertYInput) movYInput = -_verticalInputMSACC;
                    }
                }
                else
                {
                ...
                if (orbitalAtiv)
                {
                    var _minDistance = CameraSettings.OrbitalThatFollows.minDistance;
                    if (Physics.Linecast(targetTransform.position, cameras[index]._camera.transform.position,
                        out hitCamOTS))
                        if (!CameraSettings.OrbitalThatFollows.ignoreCollision)
                        {
                            distanceFromOrbitalCamera[index] =
                                Vector3.Distance(targetTransform.position, hitCamOTS.point);
                            _minDistance = Mathf.Clamp(distanceFromOrbitalCamera[index], _minDistance * 0.5f,
                                CameraSettings.OrbitalThatFollows.maxDistance);
                        }

                    xOrbit[index] +=
                        movXInput * (CameraSettings.OrbitalThatFollows.sensibility * distanceFromOrbitalCamera[index]) /
                        (distanceFromOrbitalCamera[index] * 0.5f);
                    yOrbit[index] -= movYInput * CameraSettings.OrbitalThatFollows.sensibility *
                                     (CameraSettings.OrbitalThatFollows.speedYAxis * 10.0f);
                    yOrbit[index] = ClampAngle(yOrbit[index], CameraSettings.OrbitalThatFollows.minAngleY,
                        CameraSettings.OrbitalThatFollows.maxAngleY);
                    var quaterToEuler = Quaternion.Euler(yOrbit[index], xOrbit[index], 0);
                    distanceFromOrbitalCamera[index] =
                        Mathf.Clamp(
                            distanceFromOrbitalCamera[index] -
                            movZInput * (CameraSettings.OrbitalThatFollows.speedScrool * 50.0f), _minDistance,
                            CameraSettings.OrbitalThatFollows.maxDistance);
                    var _zPosition = new Vector3(0.0f, 0.0f, -distanceFromOrbitalCamera[index]);
                    var _camNewPos = quaterToEuler * _zPosition + targetTransform.position;
                    var _camCurrentPos = cameras[index]._camera.transform.position;
                    var camRot = cameras[index]._camera.transform.rotation;
                    cameras[index]._camera.transform.rotation = Quaternion.Lerp(camRot, quaterToEuler,
                        Time.deltaTime * 5.0f * timeScaleSpeed);
                    cameras[index]._camera.transform.position = Vector3.Lerp(_camCurrentPos, _camNewPos,
                        Time.deltaTime * 5.0f * timeScaleSpeed);
                }
                else
                {
                    if (!Physics.Linecast(targetTransform.position, originalPosition[index].transform.position))
                    {
                        cameras[index]._camera.transform.position = Vector3.Lerp(
                            cameras[index]._camera.transform.position, originalPosition[index].transform.position,
                            Time.deltaTime * CameraSettings.OrbitalThatFollows.displacementSpeed);
                    }
                        ...
                        out hitCamOTS))
                    {
                        if (CameraSettings.OrbitalThatFollows.ignoreCollision)
                            ...
                            cameras[index]._camera.transform.position = Vector3.Lerp(
                                cameras[index]._camera.transform.position, hitCamOTS.point,
                                Time.deltaTime * CameraSettings.OrbitalThatFollows.displacementSpeed);
                    }
                    ...
                    if (CameraSettings.OrbitalThatFollows.customLookAt)
                    {
                        var quatLookRot =
                            Quaternion.LookRotation(
                                targetTransform.position - cameras[index]._camera.transform.position, Vector3.up);
                        cameras[index]._camera.transform.rotation = Quaternion.Slerp(
                            cameras[index]._camera.transform.rotation, quatLookRot,
                            Time.deltaTime * CameraSettings.OrbitalThatFollows.spinSpeedCustomLookAt);
                    }
                    else
                    {
                        cameras[index]._camera.transform.LookAt(targetTransform.position);
            ...
            case MSACC_CameraType.TipoRotac.ETS_StyleCamera:
                var xInputEts = _horizontalInputMSACC;
                var yInputEts = _verticalInputMSACC;
                if (CameraSettings.ETS_StyleCamera.invertXInput) xInputEts = -_horizontalInputMSACC;
                if (CameraSettings.ETS_StyleCamera.invertYInput) yInputEts = -_verticalInputMSACC;
                ...
                var newPositionETS = new Vector3(
                    originalPositionETS[index].x +
                    Mathf.Clamp(rotacXETS / 50 + CameraSettings.ETS_StyleCamera.ETS_CameraShift / 3.0f,
                        -CameraSettings.ETS_StyleCamera.ETS_CameraShift, 0), originalPositionETS[index].y,
                    originalPositionETS[index].z);
                cameras[index]._camera.transform.localPosition = Vector3.Lerp(
                    cameras[index]._camera.transform.localPosition, newPositionETS, Time.deltaTime * 10.0f);
                rotacXETS = ClampAngle(rotacXETS, -180, 80);
                rotacYETS = ClampAngle(rotacYETS, -60, 60);
                var _xQuaternion = Quaternion.AngleAxis(rotacXETS, Vector3.up);
                var _yQuaternion = Quaternion.AngleAxis(rotacYETS, -Vector3.right);
                var nextRot = originalRotation[index] * _xQuaternion * _yQuaternion;
                cameras[index]._camera.transform.localRotation = Quaternion.Lerp(
                    cameras[index]._camera.transform.localRotation, nextRot, Time.deltaTime * 10.0f * timeScaleSpeed);
                //fieldOfView
                cameras[index]._camera.fieldOfView -=
                    _scrollInputMSACC * CameraSettings.ETS_StyleCamera.speedScroolZoom * 50.0f;
                if (cameras[index]._camera.fieldOfView <
                    initialFieldOfView[index] - CameraSettings.ETS_StyleCamera.maxScroolZoom)
                    cameras[index]._camera.fieldOfView =
                        initialFieldOfView[index] - CameraSettings.ETS_StyleCamera.maxScroolZoom;
                if (cameras[index]._camera.fieldOfView > initialFieldOfView[index])
                    cameras[index]._camera.fieldOfView = initialFieldOfView[index];
            ...
            case MSACC_CameraType.TipoRotac.FlyCamera_OnlyWindows:
                var xInputFly = _horizontalInputMSACC;
                var yInputFly = _verticalInputMSACC;
                if (CameraSettings.FlyCamera_OnlyWindows.invertXInput) xInputFly = -_horizontalInputMSACC;
                if (CameraSettings.FlyCamera_OnlyWindows.invertYInput) yInputFly = -_verticalInputMSACC;
                //
                cameraRotationFly.x +=
                    xInputFly * CameraSettings.FlyCamera_OnlyWindows.sensibilityX * 15 * Time.deltaTime;
                cameraRotationFly.y +=
                    yInputFly * CameraSettings.FlyCamera_OnlyWindows.sensibilityY * 15 * Time.deltaTime;
                ...
                //
                var speedCamFly = CameraSettings.FlyCamera_OnlyWindows.movementSpeed;
                if (Input.GetKey(CameraSettings.FlyCamera_OnlyWindows.speedKeyCode)) speedCamFly *= 3.0f;
                cameras[index]._camera.transform.position += cameras[index]._camera.transform.right * speedCamFly *
                                                             Input.GetAxis(CameraSettings.FlyCamera_OnlyWindows
                                                                 .horizontalMove) * Time.deltaTime;
                cameras[index]._camera.transform.position += cameras[index]._camera.transform.forward * speedCamFly *
                                                             Input.GetAxis(CameraSettings.FlyCamera_OnlyWindows
